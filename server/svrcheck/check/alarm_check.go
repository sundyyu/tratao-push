 ...
 */
func (userAlarm *UserAlarm) CheckAlarm() {
	defer func() {
	...
	if client, err := redis.GetClient(); err == nil {
		...
		// pipes := redis.GetPipelineArr(client, max)
		// defer redis.ClosePipelineArr(pipes)
		...
		if ids, err := client.SMembers("alarm:ids").Result(); err == nil {
			...
			for _, id := range arr {
				...
				if alarms, err := redis.GetAlarmPipeline(id, pipe); err == nil {
					...
					// util.LogInfo("GetAlarmPipeline time:", (n2-n1)/1e6, "millisecond")

					// 过滤符合条件的数据
					fm := filterAlarm(alarms, userAlarm)
					// 更新Alarm的最后预警时间
 ...
 */
func filterAlarm(alarms []model.Alarm, userAlarm *UserAlarm) []model.Alarm {
	...
	for _, alarm := range alarms {
		...
		// 数据验证和处理
		if enable && len(baseCur) > 0 && len(targetCur) > 0 {
			price, _ := userAlarm.UExrate.GetPrice(baseCur, targetCur)
...
func DoSend(alarms []model.Alarm, ch *amqp.Channel, send chan int, x int, wg *sync.WaitGroup, pipe gredis.Pipeliner) {
	...
	// 更新Alarm的最后预警时间
	// if pipe != nil {
	// redis.UpdateAlarmTime(pipe, alarms)
	// }
 ...
 */
func (alarm *UserAlarm) Loop() {
	util.LogInfo("alarm loop start.")

	// cfg := config.NewConfig("../../config/cfg.yaml")
	cfg := config.GetConfig()
	t := cfg.GetInt("check.alarmTickTime") // 定时执行时间（秒）

	alarm.StopChan = make(chan int, 1)
 ...
 * @desc 停止定期器
 * @author 于朝鹏
 * @date 2018年10月8日 15:40
 */
func (alarm *UserAlarm) StopLoop() {
	alarm.StopChan <- 1
}
